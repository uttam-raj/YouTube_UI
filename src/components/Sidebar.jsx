...
function Sidebar() {
  const {showSidebar, mobileShow, setMobileShow} = useUtils();
  const sidebarItems= [
    {
      groupName:"Home",
      groupItems:[
 ...
 // Updated overlay component to cover screen and have higher z-index
 const ModelOverlay = () => {
  return (
    <div
  ...
  return (
    ...
    :"hidden h-[calc(100vh-6.625rem)] w-[18%]"
      }xl:static xl:block px-2 lg:px-6   w-[25%] h-[calc(100vh-6.625rem) overflow-hidden hover:overflow-y-auto scroll-thin`}>
      {/* Home */}
              ...
              >
                 <div
                  className={`text-xl cursor-pointer ${
                    group.groupName === "More From Youtube" ? "text-red-600" : ""
                  }`}
                >
                  {item.icon}
                </div>
